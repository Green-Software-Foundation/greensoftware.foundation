...
| — | Stat line | white | Plain text line under the logos: "130,000+ practitioners trained · ISO adoption · Global standards developed through cross-industry collaboration" |
...
- **Stat line under the logo marquee** — a plain `<p>` in `index.astro` (not a reusable component), styled to match the marquee's own caption. Three hardcoded phrases separated by "·": practitioners trained, ISO adoption, and standards developed through cross-industry collaboration
...
| Change the stat line under the logos | Edit the plain `<p>` immediately after `LogoMarquee` in `index.astro` — it's hardcoded copy, not Notion-sourced |
