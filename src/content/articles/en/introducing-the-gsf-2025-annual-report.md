---
title: "From Standards to Adoption: Introducing the GSF 2025 Annual Report"
date: "2026-01-20"
summary: "The GSF Annual Report highlights key milestones and achievements from 2025 and outlines our priorities for 2026."
teaserText: "A year in the rearview, and what’s next in 2026"
mainImage: "/assets/articles/introducing-the-gsf-2025-annual-report/main.png"
---

[**Read the 2025 Annual Report **](https://grnsft.org/annual-report-2025)

As we look back on the past year, we are proud to share our 2025 Annual Report. It reflects on GSF’s work across the ecosystem, highlights key milestones and achievements, and previews our priorities for 2026. It also helps our members, partners, and broader community stay informed about how we are advancing sustainability.

In 2025, we made progress on what matters most: building practical, consensus-based standards and tools that organizations can use to measure and reduce software’s environmental impact. 

## A Closer Look at the 2025 Annual Report

Last year was marked by several milestones, including: 

- 5 Key Projects Ratified: Impact Framework, Real Time Energy and Carbon Standard for Cloud Providers (RTC), Sustainable Organizational Framework for Technology (SOFT), Policy Radar, and Software Carbon Intensity for AI (SCI for AI). 
- Launch of the Hardware Standards Working Group. 
- New partnerships with the Government Digital Sustainability Alliance (GDSA), the World Wide Web Consortium (W3C), and the International Association of Software Architects (IASA). 
- Movement Platform launch to increase adoption through training and connection-building.  
- Green AI Position Paper release. 

Explore the full timeline, growth highlights, and what we are prioritizing next: 

[**Read the Report **](https://grnsft.org/annual-report-2025)

## Building on Our Values

Our 2025 achievements show what happens when sustainability is built in the open: standards the industry can implement, tools practitioners can use, and progress the community can verify.

## Get Involved

See what project teams delivered in 2025 and what is ahead in 2026, including work across the SCI pathway and emerging standards areas. This report shows where the industry is moving and how your organization can contribute to global sustainability efforts. 

[**Read the full Report **](https://grnsft.org/annual-report-2025)

Want to become a member? Learn how your organization can [join the GSF](https://greensoftware.foundation/join-us) and help shape the standards defining sustainable software and hardware.
